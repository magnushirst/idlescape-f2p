...
import com.idlescape.gamedata.items.equipment.AmmunitionSlotRef;
import com.idlescape.gamedata.items.equipment.EquipmentRef;
import com.idlescape.gamedata.items.equipment.OneHandedSlotRef;
import com.idlescape.gamedata.items.equipment.ShieldSlotRef;
import com.idlescape.gamedata.items.equipment.TwoHandedSlotRef;
import com.idlescape.gamedata.items.equipment.WeaponRef;
...
public class BestInSlotService {
    private static final StyledLogger LOGGER = new StyledLogger(BestInSlotService.class);
    ...
    protected Map<EquipmentSlot, List<EquipmentRef>> buildSlotItems(BisRequest request) {
        ...
        for (EquipmentRef equipment : EquipmentRef.values()) {
            ...
            slotItems
                    .computeIfAbsent(equipment.getEquipmentSlot(), key -> new ArrayList<>())
                    .add(equipment);
        }
    ...
    private boolean isAmmunition(EquipmentRef equipment) {
        if (equipment instanceof AmmunitionSlotRef) {
            AmmunitionSlotRef ammunitionSlotRef = (AmmunitionSlotRef) equipment;
            return ammunitionSlotRef.getAmmo() != null;
        }
        return false;
    }
    ...
    public static @Nullable List<AmmunitionSlotRef> getAmmoFor(WeaponRef weaponRef) {
        ...
        return Arrays.stream(AmmunitionSlotRef.values())
                ...
                .sorted(Comparator.comparingInt((AmmunitionSlotRef item) ->
                                item.getSkillsBonus().getRangedStrengthBonus())
                        .reversed()
                        .thenComparingInt(item -> item.getItemRef().getId()))
