...
public enum ToolItemRef implements GenericItemRef {
    ...
    SAW(8794),
    HAMMER(2347, "Hammer"),
    AMYS_SAW(24880, "Amy's Saw"),
    AMYS_SAW_OFF_HAND(24880, "Amy's Saw"),
    CRYSTAL_AXE(23673),
    ...
    IMCANDO_HAMMER(25644, "Imcando Hammer"),
    IMCANDO_HAMMER_OFFHAND(29775, "Imcando Hammer"),
    ...
    ToolItemRef(int id, BooleanSupplier meetsRequirements) {
    ...
    public static ToolItemRef fromId(int id) {
        for (ToolItemRef gi : values()) if (gi.id == id) return gi;
        return null;
    }

    public static int notedId(ItemRef ref) {
        return ref.getId() + 1;
    }
    ...
    public static ToolItemRef getById(int id) {
        for (ToolItemRef itemRef : values()) {
            if (itemRef.getId() == id) {
                return itemRef;
            }
        }
